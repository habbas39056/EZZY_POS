...
interface InvoiceAdjustmentViewProps {
  invoice: Invoice;
  onCancel: () => void;
...
const DISCOUNT_ACCOUNTS = [
  ...
  '40020 - Promotional Discounts',
  '50050 - Bad Debts Expense',
  '40030 - Sales Returns & Allowances',
  '50099 - Miscellaneous Expense'
];

export const InvoiceAdjustmentView: React.FC<InvoiceAdjustmentViewProps> = ({
  invoice,
  onCancel,
  onSuccess,
  currencySymbol = 'Rs'
  ...
  const currentBalance = invoice.balance ?? invoice.grossTotal ?? 0;
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const updatedInvoice: Invoice = {
      ...
      discount: (invoice.discount || 0) + adjAmt,
      status: newBalance <= 0 ? 'Completed' : invoice.status
    };
    ...
    try {
      const savedAdj = localStorage.getItem('adwiselabs_invoice_adjustments');
      const adjList = savedAdj ? JSON.parse(savedAdj) : [];
      localStorage.setItem('adwiselabs_invoice_adjustments', JSON.stringify([adjustmentRecord, ...adjList]));

      // We no longer update adwiselabs_invoices in localStorage here.
      // onSuccess will handle saving the updated invoice via API.
    } catch (err) {
    ...
    <div className="space-y-4 font-sans text-xs text-slate-700 select-none max-w-7xl mx-auto my-2">
      ...
      <div className="flex items-center justify-between bg-white px-6 py-4 rounded-lg border border-slate-200 shadow-xs">
        <div className="flex items-center gap-3">
          ...
          <h2 className="text-base font-bold text-[#0070ba]">Invoice Adjustment</h2>
        </div>
        ...
        <div className="flex items-center space-x-2.5">
          ...
          <button
            ...
            className="px-4 py-1.5 bg-[#5dade2] hover:bg-[#3498db] text-white font-bold rounded text-xs transition shadow-xs"
          >
            Apply Adjustment
          </button>
        </div>
      ...
      {/* Invoice Meta Card (Screenshot 3 Replica) */}
      <div className="bg-[#f8fafc] border border-slate-200 rounded-lg p-5">
        <div className="grid grid-cols-1 sm:grid-cols-4 gap-4 text-xs">
          <div>
            <span className="text-slate-400 block text-[11px] font-medium">Invoice No</span>
            <span className="font-semibold text-slate-800 font-mono mt-0.5 block">{invoice.invoiceNumber}</span>
          </div>

          <div>
            <span className="text-slate-400 block text-[11px] font-medium">Customer</span>
            <span className="font-semibold text-slate-800 mt-0.5 block">{invoice.customerName}</span>
          </div>

          <div>
            <span className="text-slate-400 block text-[11px] font-medium">Invoice Total</span>
            <span className="font-semibold text-slate-800 font-mono mt-0.5 block">
              {(invoice.grossTotal || 0).toLocaleString('en-US', { minimumFractionDigits: 2 })}
            </span>
          </div>
      ...
      {/* Adjustment Form Fields (Screenshot 3 Replica) */}
