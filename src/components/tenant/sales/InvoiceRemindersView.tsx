...
export const InvoiceRemindersView: React.FC<InvoiceRemindersViewProps> = ({
  ...
  const [reminders, setReminders] = useState<ReminderRecord[]>(() => {
    const saved = localStorage.getItem(`adwiselabs_invoice_reminders_${invoice.id}`);
  ...
  const handleSaveReminder = (e: React.FormEvent) => {
    ...
    localStorage.setItem(`adwiselabs_invoice_reminders_${invoice.id}`, JSON.stringify(updated));
    setIsAddingReminder(false);
    alert(`Payment reminder scheduled for invoice ${invoice.invoiceNumber}!`);
  };
    ...
    <div className="space-y-4 font-sans text-xs text-slate-700 select-none max-w-7xl mx-auto my-2">
      ...
      <div className="flex items-center justify-between bg-white px-6 py-4 rounded-lg border border-slate-200 shadow-xs">
        ...
        <span className="text-xs text-slate-500 font-mono">
          Invoice No: <strong className="text-slate-800 font-bold">{invoice.invoiceNumber}</strong>
        </span>
      </div>

      {/* ======================================================== */}
      {/* 1. INVOICE DETAILS CARD (SCREENSHOT 4 REPLICA)           */}
      {/* ======================================================== */}
      <div className="bg-white rounded-lg border border-slate-200 shadow-sm p-5 space-y-4">
        <div className="flex items-center justify-between border-b border-slate-100 pb-3">
          <h2 className="text-sm font-bold text-[#0070ba]">Invoice Details</h2>
          <button
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-6 text-xs pt-1">
          ...
          <div className="space-y-3">
            ...
            <div>
              <span className="text-slate-400 block text-[11px] font-medium">Invoice Date</span>
              <span className="font-mono text-slate-700 mt-0.5 block">{invoice.invoiceDate || 'N/A'}</span>
            </div>
          </div>

          {/* Middle: Customer & Due Date */}
          <div className="space-y-3">
            <div>
            ...
            <div>
              ...
              <span className={`font-mono mt-0.5 block ${invoice.dueDate ? 'text-slate-700' : 'text-rose-500 font-medium'}`}>
                {invoice.dueDate || 'Due Date is required to set reminder'}
              </span>
            </div>
          </div>
      ...
      {/* Add Reminder Modal / Inline Form */}
      {isAddingReminder && (
      ...
      {/* ======================================================== */}
      {/* 2. REMINDERS LIST TABLE (SCREENSHOT 4 REPLICA)           */}
      {/* ======================================================== */}
              ...
              {reminders.length === 0 ? (
                <tr>
                  <td colSpan={7} className="text-center py-8 text-slate-400">
                    No reminder records found for this invoice. Click <strong>Add Reminder</strong> to set one.
                  </td>
                </tr>
              ) : (
                reminders.map(r => (
